...
/**
 * "O que é o Tour Virtual" — mesmo modelo das seções do D.sbrave e de
 * Aplicativos: informações à esquerda (eyebrow centralizado sobre o título,
 * descrição alinhada com o primeiro card), o tour rodando num notebook à
 * direita, e os diferenciais numa fileira única em carrossel.
 */
export default function TourExplainerSection() {
  const t = useTranslations('Tour360Page.explainer');
  const tBenefits = useTranslations('Tour360Page.benefits');
  ...
  const carouselItems = useMemo(() => {
    const clonesBefore = tourBenefits.slice(-visibleCards);
    const clonesAfter = tourBenefits.slice(0, visibleCards);

    return [...clonesBefore, ...tourBenefits, ...clonesAfter];
  }, [visibleCards]);
  ...
  const handleTransitionEnd = () => {
    if (currentIndex <= visibleCards - 1) {
      setIsAnimating(false);
      setCurrentIndex(tourBenefits.length + currentIndex);
    ...
    if (currentIndex >= tourBenefits.length + visibleCards) {
      setIsAnimating(false);
      setCurrentIndex(currentIndex - tourBenefits.length);
  ...
  return (
    ...
    >
      <div className="mx-auto w-full max-w-[1800px] px-4 md:px-6">
        {/* Duas colunas: as informações à esquerda, o tour rodando num
              ...
              {carouselItems.map((item, index) => (
                ...
                >
                  <article className="flex h-full min-h-[220px] flex-col items-start gap-5 rounded-[24px] border border-[color-mix(in_srgb,var(--theme-text)_10%,transparent)] bg-[var(--theme-surface)] p-7 text-left shadow-[0_18px_44px_-26px_rgba(0,0,0,0.55)] md:p-8">
                    ...
                    >
                      <TourBeneficioIcon icon={item.icon} />
                    </span>
