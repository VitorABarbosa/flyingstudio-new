...
import DSbraveProjectCard from '../components/DSbraveProjectCard';
import { dsbraveProjects } from '../data/dsbraveData';
...
export default function DSbraveProjectsSection() {
  ...
  const carouselItems = useMemo(() => {
    const clonesBefore = dsbraveProjects.slice(-visibleCards);
    const clonesAfter = dsbraveProjects.slice(0, visibleCards);

    return [...clonesBefore, ...dsbraveProjects, ...clonesAfter];
  }, [visibleCards]);
  ...
  const handleTransitionEnd = () => {
    if (currentIndex <= visibleCards - 1) {
      setIsAnimating(false);
      setCurrentIndex(dsbraveProjects.length + currentIndex);
    ...
    if (currentIndex >= dsbraveProjects.length + visibleCards) {
      setIsAnimating(false);
      setCurrentIndex(currentIndex - dsbraveProjects.length);
